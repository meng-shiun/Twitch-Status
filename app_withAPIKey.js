$(document).ready(function() {
  ...
  function getTwitchAPI(url) {
    return $.ajax({
      ...
      url: url,
      headers: {
        'Client-ID': 'i1yf5blqxaca2tpsuzs32vgjnrmawk'  // For mengshiunlin.com
        // 'Client-ID': 'x6zp7s9nyawok2rwed72op3s15pfqs'     // For localhost test
      }
    });
  }
  ...
  function loadTwitchAPI() {
    ...
    for (let i = 0; i < allUser.length; i++) {
      ...
      $.when(
        getTwitchAPI(`https://api.twitch.tv/kraken/streams/${user}`),  // Call to check user status (Online/ Offline)
        getTwitchAPI(`https://api.twitch.tv/kraken/channels/${user}`)  // Call to fetch other information
      )
